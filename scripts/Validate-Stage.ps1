...
switch ($Stage) {
  ...
  "rbac" {
    ...
    foreach ($f in $hubFiles) {
      ...
      if (-not $principalId) { throw "UAMI missing principalId: $($u.name)" }
      ...
      if ($null -eq $ra -or $ra.Count -eq 0) { throw "RBAC missing Contributor for $($u.name) at $scope" }
  ...
  "nva" {
    ...
    foreach ($f in $hubFiles) {
      ...
      if ($null -eq $apps -or $apps.Count -eq 0) { throw "No NVA managed app found in $($h.resourceGroup.name) for $($h.name)" }
  ...
  "routing_intent" {
    ...
    foreach ($f in $hubFiles) {
      ...
      if ($null -eq $ri -or $ri.Count -eq 0) { throw "No RoutingIntent found in RG $($h.resourceGroup.name) for $($h.name)" }
